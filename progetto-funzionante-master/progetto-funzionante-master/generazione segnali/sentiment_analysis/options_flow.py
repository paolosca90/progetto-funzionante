...
class OptionsFlowAnalyzer:
    ...
    async def fetch_options_flow(self, symbols: List[str], hours_back: int = 4) -> List[OptionsFlowData]:
        ...
        try:
            for symbol in symbols:
                flows = await self._generate_simulated_flow(symbol, hours_back)
    ...
    async def _generate_simulated_flow(self, symbol: str, hours_back: int) -> List[OptionsFlowData]:
        """Generate realistic simulated options flow"""
        flows = []
        
        # Simulate underlying price based on symbol
        base_prices = {
            "SPX500_USD": 4850.0,
            "NAS100_USD": 17500.0,
            "US30_USD": 38500.0,
        ...
        underlying_price = base_prices.get(symbol, 100.0)
        
        # Generate realistic flow patterns
        num_flows = np.random.randint(10, 50)  # 10-50 flows per symbol
        
        for i in range(num_flows):
            flow = self._create_realistic_flow(symbol, underlying_price, hours_back)
            flows.append(flow)
        
        return flows

    def _create_realistic_flow(self, symbol: str, underlying_price: float, hours_back: int) -> OptionsFlowData:
        """Create single realistic options flow"""
